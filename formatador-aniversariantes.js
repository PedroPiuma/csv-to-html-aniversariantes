...
const writeHtml = () =>
  writeFile("conteudo-final.html", htmlTextFinal.trim(), (error) =>
    error
      ? new Error(error)
      : console.log(
          "Script executado com sucesso! Verifique o arquivo conteudo-final.html :)\n"
        )
  )
const readCsv = (formatText) =>
  readFile("aniversariantes.csv", "utf-8", (error, text) =>
    error ? new Error(error) : formatText(text)
  )
const createPhoto = (firstName, lastName) =>
  `<td width="150"><img style="width: 120px; height: 160px; display: block; margin-left: auto; margin-right: auto; border-radius: 10%; border: 1px solid; border-color: #ab9b6a;" src="https://arquivos.essentialnutrition.com.br/images/aniversarios/${thisYear}/outubro/${firstName
    .normalize("NFD")
    ...
    .replace(
      /[\u0300-\u036f]/g,
      ""
    )}.jpg" alt="${firstName} ${lastName}" title="${firstName} ${lastName}" /></td>`
const createPersonDescription = (
  firstName,
  lastName,
  sector,
  local,
...
const upperCaseFormatterSector = (text) =>
  text.length <= 3
    ...
    : text
        ...
        .map((word) =>
          word.length > 3
            ? upperCaseFormatterName(word.toLowerCase())
            : word.toLowerCase()
        )
        .join(" ")
...
const formatText = (text) => {
  ...
  const createRows = () =>
    objectPerson.map((el, index) => {
      ...
      const lastName = upperCaseFormatterName(
        names[names.length - 1].toLowerCase()
      )
      const conectorName =
        names[names.length - 2]?.length === 2
          ? names[names.length - 2].toLowerCase()
          : false
  ...
  const matchRows = () => {
    ...
    const numberOfRows =
      imageRows.length === descriptionRows.length
        ? imageRows.length
        : new Error("Image rows length is diferent of description rows length.")
