...
$file = new \SplFileObject("teste.csv");
...
foreach ($array_final_final_mesmo as $key => $item) {
    ...
    foreach($item as $value){
        echo gerar_valores($value['firstname'], $value['lastname'], $value['birthday'], $value['sector'], $value['local']);
    }
    echo "</tr>\r\n";
}
...
function gerar_fotos($firstname, $lastname)
{
    return  '
            ...
            <img style="width: 120px; height: 160px; display: block; margin-left: auto; margin-right: auto; border-radius: 10%; border: 1px solid; border-color: #ab9b6a;"
                src="https://arquivos.essentialnutrition.com.br/images/aniversarios/2021/12-dez/' . ucfirst($firstname) . ucfirst($lastname) . '.jpg"
                alt="" />
            </td>';
}

function gerar_valores($firstname, $lastname, $birthday, $sector, $local)
{
    return 
            '  <td width="150">
                  <p style="text-align: center; font-family: arial, sans-serif; color: #1a1a1a; font-size: 12px; line-height: 18px; margin-bottom: 30px;">
                    <b>Dia ' . (int)$birthday . '<br /></b>' . ucfirst($firstname) . ' ' . ucfirst($lastname) . '<br /> ' . ucfirst(mb_strtolower($sector)) . ' <br /> ' . $local . '
                  </p>
              </td>';
}
